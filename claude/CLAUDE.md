Global rules and conventions. Detailed instructions live in `agent-docs/` and are loaded on demand — read the relevant file when a trigger matches.
...
`agent-docs/how-to-plan.md` - (For when planning, entering plan mode, or for non-trivial tasks (3+ steps))
`agent-docs/statusline.md` - (For when modifying statusline.sh, context-tracker.sh, or the statusline visual design)
`agent-docs/cli-tools.md` - (For when doing code refactoring, benchmarking, diffing, or config file processing via Bash)
